...
export const UserUpdataAddress = async (req, res, next) =>{
    const {
       ...
       try{
        const saveUpdataAddress = await ADDAddress.findByIdAndUpdate(adreid,{
             $set:{   
               nameaddress:naddress,
               tel:tels,
               address:uaddress,
 ...
 export const allAddress = async (req, res, next) =>{
     const userId = req.params.userid
     try{
       const alladdressuser = await ADDAddress.find({mangauser:userId}).populate("mangauser")
 ...
 export const OneAddress = async (req, res, next) =>{
     ...
     try{
       const alladdressuser = await ADDAddress.findById(addresId).populate("mangauser")
       res.status(200).json(alladdressuser)
     }catch(err){
       next(err)
     }
 ...
 export const DeletAddress = async (req, res, next) =>{
     const addresId = req.params.adreid
     try{
       const alladdressuser = await ADDAddress.findByIdAndDelete(addresId).populate("mangauser")
